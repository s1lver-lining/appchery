...
# Not named COLUMNS: bash keeps the terminal width in that name and rewrites it on every window
# size change, so an interactive run would hand vitest a number instead of a path.
DUMP=$(mktemp)
docker exec "$CONTAINER" psql -X -A -t -U postgres -c \
	"copy (select table_name, column_name, is_nullable, case when column_default is null then 'NO' else 'YES' end
		...
		to stdout with csv" > "$DUMP"
APPCHERY_SERVER_COLUMNS="$DUMP" npx vitest run src/lib/db/synced.schema.test.ts --reporter=dot
rm -f "$DUMP"
