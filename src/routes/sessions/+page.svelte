...
<div class="safe-top mx-auto w-full max-w-2xl p-4 pt-6">
	<h1 class="mt-2 mb-4 text-2xl font-bold tracking-tight">{$t('sessions.title')}</h1>
					...
					>
						<div class="min-w-0">
							...
							<p class="text-sm text-muted">
								{$formatDayDateTime(s.startedAt)} · {activityLabel(s.id)}
							</p>
						</div>
	...
	<!-- Below the list, where the thumb already is after scrolling through it. -->
	<button
		class="mt-4 flex w-full items-center justify-center gap-1.5 rounded-xl bg-brand py-3 font-semibold text-brand-ink"
		onclick={start}
	>
